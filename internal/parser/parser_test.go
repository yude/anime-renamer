...
func TestExtractLastSubtitle(t *testing.T) {
	tests := []struct {
		name         string
		input        string
		wantName     string
		...
		{
			name:         "multiple brackets - last one wins",
			input:        "作品【前】ep.7「タイトル」",
			wantName:     "作品【前】ep.7",
			wantSubtitle: "タイトル",
		},
		{
			name:         "subtitle with Japanese",
			input:        "作品 ep.7「さくらんぼキッス〜爆発だ!〜」",
			wantName:     "作品 ep.7",
			wantSubtitle: "さくらんぼキッス〜爆発だ!〜",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			subtitle := ""
			got := extractLastSubtitle(tt.input, &subtitle)
			if got != tt.wantName {
				t.Errorf("extractLastSubtitle() name = %q, want %q", got, tt.wantName)
			}
			if subtitle != tt.wantSubtitle {
				t.Errorf("extractLastSubtitle() subtitle = %q, want %q", subtitle, tt.wantSubtitle)
			}
		})
	}
}
